module p_ls2
  ...
  !! line-sink elements with resistance. Elements are defined as strings of
  !! points, with the specified head and bottom elevation at each vertex.
...
contains
  ...
  subroutine LS2_SetupFunctions(io, ls2, fls)
    ...
    !! by the matrix solution and applied via LS2_Update.
    !!
    !! Calling Sequence:
    !!    call LS2_SetupFunctions(io, ls2, fls)
    !!
    !! Arguments:
    ...
    if (io%lDebug) then
      call IO_Assert(io, (associated(ls2)), &
           "LS2_SetupFunctions: LS2_Create has not been called")
      call IO_Assert(io, (associated(fls)), &
           "LS2_SetupFunctions: Illegal FLS_COLLECTION object")
    end if
    ...
    do iStr = 1, ls2%iNStr
      ...
      do iVtx = 1, str%iNPts-1
        ...
        next_vtx => str%Vertices(iVtx+1)
        this_vtx%rLength = abs(next_vtx%cZ - this_vtx%cZ)
        this_vtx%pFLS => FLS_New(io, fls, this_vtx%cZ, next_vtx%cZ, cZERO, ELEM_LS2, iStr, iVtx, -1)
      end do
    end do
  ...
  subroutine LS2_SetupMatrix(io, ls2, mat)
  ...
  function iLS2_Prepare(io, ls2, aem, iIteration) result(iChanges)
  ...
  subroutine LS2_ComputeCoefficients(io, ls2, aem, fls, cPathZ, iEqType, iElementType, iElementString, &
    ...
    !!
    !! Calling Sequence:
    !!    call LS2_ComputeCoefficients(io, ls2, aem, fls, cPathZ, iEqType, cOrientation, rRow)
    !!
    !! Arguments:
    !!   (in)    type(LS2_COLLECTION), pointer
    !!             LS2_COLLECTION object to be used
    !!   (in)    type(AEM_DOMAIN), pointer
    !!             AEM_DOMAIN object to be used
    !!   (in)    type(FLS_COLLECTION), pointer
    ...
    type(IO_STATUS), pointer :: io
    ! [ LOCALS ]
    integer(kind=AE_INT) :: iStat, iCol, iStr, iVtx, iNLS, iWhich
    ...
    if (io%lDebug) then
      ...
      call IO_Assert(io, (associated(fls)), &
           "LS2_ComputeCoefficients: Illegal FLS_COLLECTION object")
    end if
    ...
    do iStr = 1, ls2%iNStr
      ...
      ! ASSUMES that LS2_SetupFunctions created consecutive linesink entries
      iNLS = str%iNPts-1
  ...
  function rLS2_ComputeRHS(io, ls2, aem, iEqType, iElementType, iElementString, iElementVertex, &
    ...
    !!
    !! Calling Sequence:
    !!   rRHS = rLS2_ComputeRHS(io, ls2, aem, rValue, iElementType, iElementString, iElementVertex, &
         !!                          iElementFlag)
    !!
    !! Arguments:
    !!   (in)    type(LS2_COLLECTION), pointer :: ls2
  ...
  subroutine LS2_StoreResult(io, ls2, rValue, iElementType, iElementString, iElementVertex, iElementFlag, lDirect)
    ...
    ! [ LOCALS ]
    type(LS2_STRING), pointer :: str
    type(LS2_VERTEX), pointer :: this_vtx
  ...
  subroutine LS2_Update(io, ls2, fls)
    ...
    if (io%lDebug) then
      ...
      call IO_Assert(io, (associated(fls)), &
           "LS2_Update: Illegal FLS_COLLECTION object")
    end if
  ...
  subroutine LS2_ComputeCheck(io, ls2, aem, lLinearize)
    ...
    do iStr = 1, ls2%iNStr
      do iVtx = 1, ls2%Strings(iStr)%iNPts - 1
        ...
        if (lLinearize) then
          ...
          if (.not. lDOM_IsConfined(io, aem%dom, vtx%cCPZ(1), vtx%rCheckPot)) ls2%iRegenerate = 1
        end if
      end do
    end do
  ...
  function iLS2_DoRouting(io, ls2, iCurrentIteration, lSwitch, lReportFlows) result(iChanges)
    ...
    if (iCurrentIteration > 2) then
      do iStr = 1, ls2%iNStr
        ...
        if (str%lRoute .and. str%iDownstreamID < 0) then
          rStreamFlow = rZERO
          iChanges = iChanges + iLS2_RouteString(io, ls2, str%iID, rStreamFlow, lSwitch, 0)
  ...
  recursive function iLS2_RouteString(io, ls2, iID, rStreamFlow, lSwitch, iChangesIn) result(iChanges)
    ...
    do iStr = 1, ls2%iNStr
      ...
      if (this%iID == str%iDownstreamID) then
        rSQi = rZERO
        iChanges = iLS2_RouteString(io, ls2, str%iID, rSQi, lSwitch, iChanges)
    ...
    do
      ...
      select case (iOpCode)
        ...
        case (kOpData)
          ...
          cZ = cIO_GetCoordinate(io, sTag, extents=.true., check_points=str%Vertices(1:str%iNPts)%cZ)
  ...
  subroutine LS2_Report(io, ls2, aem)
    ...
    if (associated(ls2%Strings)) then
      ...
      call HTML_AttrInteger('Number of FLS functions', iLS2_GetInfo(io, ls2, SIZE_FLS, 0))
      ...
      do iStr = 1, ls2%iNStr
        ...
        call HTML_StartTable()
        call HTML_TableHeader((/ 'Vertex', 'FLS # ', 'X     ', 'Y     ', 'Length', &
        ...
        do iVtx = 1, str%iNPts-1
          ...
          call HTML_ColumnReal((/vtx%rLength, vtx%rCPHead, str%rConductance, vtx%rCPDepth, &
          	   vtx%rStrength, rDOM_PotentialToHead(io, aem%dom, vtx%rCheckPot, vtx%cCPZ(1))/))
  ...
  subroutine LS2_Load(io, ls2, fls, mode)
