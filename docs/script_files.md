...
## The ModAEM name file `modaem.nam` 
...
The first line of the name file contains the “base” name of the files for the model run. For example, if the model input data are contained in a file called `modaem.aem`, then the contents of `modaem.nam` would be:
...
The various processing directives that are available once a problem has been defined in the AEM section are described in Chapter [cha:Processing-Directives].
